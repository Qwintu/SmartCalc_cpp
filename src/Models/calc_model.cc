...
double CalcModel::CalcExpression() {
  // std::cout << "CalcExpression Str: " << input_string << " x: " << x << std::endl;

  char c;  ///<  символ строки, который обрабатаваем
  int open_parenthese_flag = 0;
  double value = 0;
  double to_stack_value = 0;
  stack_dbl stack_dbl;  ///<  инициализация стэка с операндами
  stack_char stack_char;  ///<  инициализация стэка с операторами

  /*!
   * Основной цикл парсинга входной строки выражения
   */
  for (int i = 0; input_string[i] != '\0'; i++) {
    c = parse_func(input_string[i], input_string[i + 1], &i);
    if (isNumber(c)) {

      to_stack_value = parse_number(input_string, &i);
...
double CalcModel::calc_top_stack_unaru(stack_char *stack_char, stack_dbl *stack_dbl) {
  ...
  switch (stack_c) {
    case 'c':
      if(rad_deg_fl == 0) a = grad_to_rad(a);
      res = cos(a);
      break;
    case 's':
      if(rad_deg_fl == 0) a = grad_to_rad(a);
    ...
    case 'o':
      // a = grad_to_rad(a);
      res = acos(a);
      if(rad_deg_fl == 0) res = rad_to_grad(res);
      break;
    case 'i':
      // a = grad_to_rad(a);
      res = asin(a);
...
int CalcModel::parse_func(char ch, char next_c, int *i) {
  // char *numbs_string = "0123456789.+-*/:()";

  // std::string numbers_string = "0123456789.+-*/:()";
  // char *numbs_string = &(*numbers_string.begin());

  if (!strchr("0123456789.+-*/:()", ch)) {  // если не цифра или знак меняем с и
...
double CalcModel::calc_top_stack_binary(stack_char *stack_char, stack_dbl *stack_dbl) {
...
int CalcModel::take_priority(char c) {
  int priority_val = 0;
...
double CalcModel::parse_number(std::string string, int *i) {
  double parsed_value = 0;
  char temp_value[256] = {'\0'};

  // char *digits = "0123456789.";
  // char *begin = (char *)string + *i;   // C version
  char *begin = &(*string.begin()) + *i;

  size_t len_numb =
      strspn(begin, "0123456789.");  // Вычисляет длину начального сегмента str1,
...
double CalcModel::grad_to_rad(double a) { return ((a * PI) / 180); }

double CalcModel::rad_to_grad(double a) { return ((a * 180) / PI); }

void CalcModel::push_stack_dbl(stack_dbl *stack, double value) {
  stack->stack_dbl[stack->stack_dbl_count] = value;
  stack->stack_dbl_count++;
}

void CalcModel::push_stack_char(stack_char *stack, double value) {
  stack->stack_char[stack->stack_char_count] = value;
  stack->stack_char_count++;
}

double CalcModel::pop_stack_dbl(stack_dbl *stack) {
  stack->stack_dbl_count--;
...
char CalcModel::pop_stack_char(stack_char *stack) {
  stack->stack_char_count--;
...
int CalcModel::is_empty_stack_dbl(stack_dbl *stack) { return stack->stack_dbl_count == 0; }

int CalcModel::is_empty_stack_char(stack_char *stack) {
  return stack->stack_char_count == 0;
}

double CalcModel::top_stack_dbl(stack_dbl *stack) {
  return stack->stack_dbl[stack->stack_dbl_count - 1];
}
char CalcModel::top_stack_char(stack_char *stack) {
  if (stack->stack_char_count == 0) return 0;
  return stack->stack_char[stack->stack_char_count - 1];
}

int CalcModel::isNumber(char c) {
  return ((c > 47 && c < 58) ||
